...
class Lesson extends Component {
  render () {
    ...
    return (
      <div onClick={() => console.log(`${this.props.index} - ${lesson.title}`)}>//这里还是有一点不懂
        <h1>{lesson.title}</h1>
...
class LessonsList extends Component {
  render () {
    return (
      // <div>
      //   {this.props.lessons.map((lesson, i) => <Lesson key={i} lesson={lesson} />)}
      // </div>
      //上面是我自己写的，下面是正确答案，主要是定义了一个函数，返回了index
